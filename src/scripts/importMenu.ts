...
const client = createClient({
  ...
  token: 'skq76uwtqr3wIpUUiWdG1MzQ6sAg1vDQUzXSefvuu4gNEBh5ql62oo4xL42HDwUMH9PCxNNvaJn8KfnvtBMKN82W1LK743hv5pFecfog00k6gD8OsGZqZSCeleCjaykpPJo3HJekroz7Fr2462HhXv9ambNoYO6900MIcMyWbebSIwbSDq4y', // safer to use env var
  useCdn: false,
});
...
// Create categories
// ---------------------------
async function createCategories(categoryNames: Record<string, any>) {
  const categoryDocs = Object.entries(categoryNames).map(([slug, names], index) => ({
    _type: 'category',
    _id: slug,
    name: names,
    slug: { _type: 'slug', current: slug },
    order: index, // preserve order from your object
  }));

  for (const category of categoryDocs) {
    try {
      await client.createIfNotExists(category);
      console.log(`✅ Category created: ${category.slug.current}`);
    } catch (err) {
      console.error('Error creating category:', category.slug.current, err);
    }
  }
}
...
// Upload menu items
// ---------------------------
async function uploadMenuItems(MENU: any[]) {
  for (let index = 0; index < MENU.length; index++) {
    ...
    const doc: any = {
      ...
      order: index, // preserves the order in the MENU array
    };

    // handle price / sizes
    if (item.price) doc.price = item.price;
    if (item.sizes) doc.sizes = item.sizes;
    ...
    if (item.image) {
      ...
      if (fs.existsSync(imagePath)) {
        try {
          ...
          console.error('Error uploading image:', item.image, err);
        }
      } else {
        console.warn('Image not found, skipping:', item.image);
      }
    }

    try {
      await client.create(doc);
      console.log(`✅ Menu item created: ${item.name.en}`);
    } catch (err) {
      console.error('Error creating menu item:', item.name.en, err);
    }
  }
}
...
async function main() {
  ...
  await createCategories(categoryNames);

  // then menu items
  await uploadMenuItems(MENU);
